 ...
 * Title: Battle 
 * 
 * Description: This class manages the turn-based battle aspect of the game for enemies.
 * It acts as a super class for FinalBossBattle, as many similar aspects in this class
 * can be found in the final boss battle. It includes many
 * components. 
 * -A gameState variable to determine what state the battle is in. 
 * -A protagonistTurn method that determines the user's command (Attack, Defend, Run away) and calls the necessary 
 * classes to get the results of said commands and performs adjustments with the health points of
 * the enemy and the battle points and defense power of the protagonist. 
 * -A enemyTurn method that does the calculations 
 * to get the results of the enemy's attack and performs adjustments with the health points of
 * the protagonist.  
 * -A death method that ends the game if the protagonist dies
 * 
 * @author Aalea Ally
 * 
 */
public class Battle extends JFrame {
	...
	public Battle(Protagonist protagonist, Enemy enemy, FinalBoss boss) {
		...
		if (enemy != null) { //if this is an enemy battle
			...
			JOptionPane.showMessageDialog(this, "You have encountered an enemy!", null, JOptionPane.PLAIN_MESSAGE, enemy.sprite[3][1]);
			...
			image = boss.sprite[0][0].getImage(); //convert imageicon to image
			fixedImage = image.getScaledInstance(125, 220, java.awt.Image.SCALE_SMOOTH); //scale image
			boss.sprite[0][0] = new ImageIcon(fixedImage); //convert the new image to imageicon and assign it back
			
			//display dialog with boss standing as an image
			JOptionPane.showMessageDialog(this, "You have encountered the boss!", null, JOptionPane.PLAIN_MESSAGE, boss.sprite[0][0]);
			//display dialog with protagonist's health points and a red heart
			JOptionPane.showMessageDialog(this, "Your health points: " + protagonist.getHealthPoints(), null, JOptionPane.PLAIN_MESSAGE, new ImageIcon("images/heart.png"));
	...
	private void resizeSprites(Image image, Image fixedImage) {
		...
		image = protagonist.sprite[2][1].getImage(); //convert the imageicon to an image
		fixedImage = image.getScaledInstance(200, 240, java.awt.Image.SCALE_SMOOTH); //scale the image
		protagonist.sprite[2][1] = new ImageIcon(fixedImage); //assign the new image to the imageicon of the protagonist punching sprite
		
		//resize the imageicon of the protagonist using the slingshot
		image = protagonist.sprite[3][1].getImage(); //convert the imageicon to an image
		fixedImage = image.getScaledInstance(220, 220, java.awt.Image.SCALE_SMOOTH); //scale the image
		protagonist.sprite[3][2] = new ImageIcon(fixedImage); //assign the new image to the imageicon of the protagonist using the slingshot

		//resize the imageicon of the protagonist using the flail
		image = protagonist.sprite[4][1].getImage(); //convert the imageicon to an image
		fixedImage = image.getScaledInstance(223, 387, java.awt.Image.SCALE_SMOOTH); //scale the image
		protagonist.sprite[2][2] = new ImageIcon(fixedImage); //assign the new image to the imageicon of the protagonist using the flail
		...
		try {
			image = enemy.sprite[5][0].getImage(); //convert the imageicon to an image
			fixedImage = image.getScaledInstance(220, 220, java.awt.Image.SCALE_SMOOTH); //scale the image
			enemy.sprite[5][0] = new ImageIcon(fixedImage); //assign the new image to the imageicon of the enemy getting hurt
			
			image = enemy.sprite[3][1].getImage(); //convert the imageicon to an image
			fixedImage = image.getScaledInstance(220, 220, java.awt.Image.SCALE_SMOOTH); //scale the image
			enemy.sprite[3][1] = new ImageIcon(fixedImage); //assign the new image to the imageicon of the enemy standing
		}
		...
		image = protagonist.sprite[6][1].getImage(); //convert the imageicon to an image
		fixedImage = image.getScaledInstance(371, 220, java.awt.Image.SCALE_SMOOTH); //scale the image
		protagonist.sprite[6][1] = new ImageIcon(fixedImage); //assign the new image to the imageicon of the protagonist on the floor

		//resize the imageicon of the protagonist hurt
		image = protagonist.sprite[5][1].getImage(); //convert the imageicon to an image
		fixedImage = image.getScaledInstance(255, 240, java.awt.Image.SCALE_SMOOTH); //scale the image
		protagonist.sprite[5][1] = new ImageIcon(fixedImage); //assign the new image to the imageicon of the protagonist hurt
	...
	public void protagonistTurn() {
		...
		if (chosenCommand.equals("Attack")) { //if the user wishes to attack
			...
			if (attackType.equals("Punch (no battle points)")) { //if the user chose the punch attack
				...
				JOptionPane.showMessageDialog(this, "Click OK when you think you're ready to hit", null, JOptionPane.PLAIN_MESSAGE, protagonist.sprite[2][1]);
			}
			else if (attackType.equals("Slingshot (1 battle point)")) { //if the user chose the slingshot attack
																	   //display a dialog box prompting when to attack (when to stop the timer)
				JOptionPane.showMessageDialog(this, "Click OK when you think you're ready to release", null, JOptionPane.PLAIN_MESSAGE, protagonist.sprite[3][1]);
			}
			else if (attackType.equals("Flail (5 battle points)")) { //if the user chose the flail attack
																	//display a dialog box prompting when to attack (when to stop the timer)
				JOptionPane.showMessageDialog(this, "Click OK when you think you're ready to swing", null, JOptionPane.PLAIN_MESSAGE, protagonist.sprite[4][1]);
			}
			...
			JOptionPane.showMessageDialog(this, "You inflicted " + attackPower
					+ " damage on the enemy!", null, JOptionPane.PLAIN_MESSAGE, enemy.sprite[5][0]); //display message of how much damage
			...
			if (hitMeter.canRunAway()==true) { //if the hit meter returns true
				...
				JOptionPane.showMessageDialog(this, "HAH you tripped while attempting to run away", null, JOptionPane.PLAIN_MESSAGE, 
						protagonist.sprite[6][1]); //display message and image of protagonist on floor
	...
	public void enemyTurn() {
		...
		JOptionPane.showMessageDialog(this, "The enemy attacked you! You lost "
				...
				JOptionPane.PLAIN_MESSAGE, protagonist.sprite[5][1]); //display message indicating the enemy's attack power, 
