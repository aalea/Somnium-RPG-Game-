 ...
 * 
 * Description: This class manages the turn-based battle aspect of the 
 * game for the final boss. It acts as a parent class to Battle. It includes many
 * components. 
 ...
 * the boss and the battle points and defense power of the protagonist. 
 * -A bossTurn method that determines the boss' command at random and does the calculations 
 * to get the results of said commands and performs adjustments with the health points of
 * the protagonist and the defense power of the boss. 
 * -A friendsTurn method that determines how many friends the user did not kill and generates the attack power
 * for each and adjusts the boss' health points. 
 * -An end method that stops the boss battle music, plays the ending music, and calls the Ending class
 * 
 * Areas of Concern: It is mainly the fault of the room class, however
 * the room music continues to play when entering the final battle at 
 * the same time the boss battle music is playing.
 * 
 * @author Aalea Ally
 * 
 */
public class FinalBossBattle extends Battle {
	...
	public void protagonistTurn() {
		...
		//12. Check if the player clicked on the defend button and the game state is 1
			...
			JOptionPane.showMessageDialog(this, "HAH you tripped while attempting to run away", null, JOptionPane.PLAIN_MESSAGE, protagonist.sprite[6][1]);
	...
	public void bossTurn() {
		...
		if (attackChoice <= 2) { //if the random number is 1 or 2
			...
			JOptionPane.showMessageDialog(this, "The boss punched you! You lost "
					...
					JOptionPane.PLAIN_MESSAGE, IntroScreen.boss.sprite[1][0]); //display boss' attack power and protagonist' health after attack
			...
			JOptionPane.showMessageDialog(this, "The boss jumped on you! You lost "
					+ bossAttackPower + " health points. You now have "
					+ protagonist.getHealthPoints() + " health points.", null, 
					JOptionPane.PLAIN_MESSAGE, IntroScreen.boss.sprite[2][0]); //display boss' attack power and protagonist' health after attack
	...
	public void friendsTurn(int friendsAmount) {
		...
		JOptionPane.showMessageDialog(this, "Your "
				...
				JOptionPane.PLAIN_MESSAGE, enemy.sprite[3][1]); //display message showing how many friend 
															  //the protagonist didn't kill with their sprite
		...
		for (int friendCounter = friendsAmount; friendCounter > 0; friendCounter--) { //continue until all friends have done their attack
			...
			if (friendCounter >= 4) { //if the 4th or higher friend is attacking (can't do same for all as 3 uses 3rd, 2 uses 2nd, 1 uses 1st)
				...
				JOptionPane.showMessageDialog(this, "Your "
						...
						IntroScreen.boss.sprite[4][0]); //display message of the current friend's damage to the boss and the boss hurt sprite
			}
	...
	public void end() {
		...
		FrameSetup.playSound("STOP", ""); //stop any excess music playing in framesetup
