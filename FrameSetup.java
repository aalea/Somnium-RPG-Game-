 ...
 * Title: Frame setup
 * 
 * Description: This is the class that setups the JFrame and all of its 
 * components. It also transitions the intro screen to the room, controls
 * switching between rooms, and displays messages the user touches. 
 * 
 * @author Aalea Ally
 *
 */
public class FrameSetup extends JFrame implements KeyListener {
	...
	public static void displayMessage(int indexOfMessage) {
		...
		if (RPGGameTest.paper[indexOfMessage].isWasRead()==false) {
			...
			RPGGameTest.paper[indexOfMessage].setWasRead(true); //makes it so that the message can't be viewed again
	...
	public void switchRooms(int currentRoom, int nextRoom) {
		...
		try {
			...
			System.out.println("the system does not have enough memory to play the room music");
		}
	...
	static void playSound(String name, String destination) {
		...
		if (name.equals("STOP")) { //stop the music if "STOP" is sent to this method
			...
			clip.close(); //empties the audio loaded into the memory
